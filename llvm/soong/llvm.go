...
func forceBuildLlvmComponents(ctx android.LoadHookContext) {
	forceBuild := false
	if ctx.AConfig().IsEnvTrue("FORCE_BUILD_LLVMX_COMPONENTS") {
		forceBuild = true
	}
...
func init() {
	android.RegisterModuleType("xllvm_defaults", llvmDefaultsFactory)
	android.RegisterModuleType("xforce_build_llvm_components_defaults", forceBuildLlvmComponentsDefaultsFactory)
}
