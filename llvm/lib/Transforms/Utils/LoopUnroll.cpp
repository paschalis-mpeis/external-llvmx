...
LoopUnrollResult llvm::UnrollLoop(Loop *L, UnrollLoopOptions ULO, LoopInfo *LI,
  ...
  // Apply some post-loop optimizations based on special bitcode annotation
  const bool dbg_ = false;
  if((dbg_)) {
    outs() <<   "\n\nFunction: " << OriginalLoopBlocks.at(0)->getParent()->getName()
      << "\nResult: Unrolled: " << (CompletelyUnroll? "fully":"partially")
      << "\nOriginal Blocks: " << OriginalLoopBlocks.size() << " After unrolling: " << L->getNumBlocks()
      << "\n\n";
  }

  for (BasicBlock *BB : L->getBlocks()) {
    bool original=false;
    for (auto *origBB : OriginalLoopBlocks) {
      if(origBB->getName().compare(BB->getName()) ==0) {
        original=true;
        break;
      }
    }

    // with some static analysis we could also eliminiate some bound checking
    // in some specific loop cases
    bool boundCheckElimination = false;
    llvm::Instruction *toDeleteBC=nullptr;
    llvm::Instruction *toDeleteSC=nullptr;
    if(!original) {
      for (Instruction &I : *BB) {
        ...
        if(isa<CallInst>(I)) {
          ...
          if(F!=nullptr) {
            if (MDNode* N = F->getMetadata("android.check.suspend")) {
              toDeleteSC=&I;
            } else if (MDNode* N = F->getMetadata("android.check.bounds")) {
              toDeleteBC=&I;
            }
          }
        }
    ...
    if(toDeleteSC) {
      info+=" SuspendCheck";
      toDeleteSC->eraseFromParent();
      if(dbg_) outs() << "Deleted SuspendCheck from: " << BB->getName() << "\n";
    }

    if(boundCheckElimination && toDeleteBC) {
      info+=" BoundsCheck";
      toDeleteBC->eraseFromParent();
    }

    if((dbg_)) {
      if(original) {
        outs() << "Original: " << BB->getName() << info << "\n";
      } else {
        outs() << "Unrolled: " << BB->getName() << info << "\n";
      }
    }
  }
